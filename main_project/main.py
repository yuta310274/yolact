...
def parse_args(argv=None):
    ...
    parser.add_argument('--ap_data_fi0le', default='results/ap_data.pkl', type=str,
    ...
    parser.add_argument('--images', default=None, type=str,
                        help='An input folder of images mat is not a numpy array, neither a scalarand output folder to save detected images. Should be in the format input->output.')
...
def run_subprocess():
    # ボックスと推定重量を文字列に変換
    #boxes_str = ' '.join(map(str, boxes))
    #estimated_weights_str = ' '.join(map(str, estimated_weights))
    # 別のプログラムを実行する
    
    subprocess.run(["python", "sample_6.py"])


    #非同期処理
    #subprocess.Popen(["python", "sample_6.py"])
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    if undo_transform:
        ...
        cv2.imwrite("img.png",img_numpy)
        img_gpu = torch.Tensor(img_numpy).cuda()
    else:
        img_org = img.to('cpu').detach().numpy().copy()
        cv2.imwrite("img.png",img_org)
        img_gpu = img / 255.0
        h, w, _ = img.shape
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        ...
        instance_number=1
        mask_number=1
       # with open('data_strawberry/data/bright_dark/yumenoka/result20/strawberry_20.csv', 'w') as f:
        #with open('../yumenoka/result26/strawberry_1.csv', 'w') as f:

        for mask in masks:
            mask_np=mask.to('cpu').detach().numpy().copy()
            #cv2.imshow("mask",mask_np)
            #cv2.waitKey(0)
            mask_np_1=np.clip(mask_np * 255, 0, 255).astype(np.uint8)
            #cv2.imwrite(f"mask_{frame_number}_{mask_number}.png",mask_np_1)
            #cv2.imwrite(f"./data_strawberry/data/bright_dark/yumenoka/result20/mask_{mask_number}.png",mask_np_1)
            cv2.imwrite(f"../mask_directory/mask_{mask_number}.png",mask_np_1)
            #cv2.imwrite(f"mask_{mask_number}.png",mask_np_1)
            instance_numbers.append(instance_number)
            mask_numbers.append(mask_number)
            #print(mask_np.dtype)
            mask_nps.append(mask_np)
            mask_np=mask_np.astype(np.uint8)
            #cv2.imwrite(f"mask_{frame_number}_{mask_number}.png",mask_np)
            #cv2.imshow("mask",mask_np)
            #cv2.waitKey(0)
            #mask_nps.append(mask_np)
            contours, hierarchy = cv2.findContours(mask_np, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_TC89_L1)
            for i, c in enumerate(contours):
                area = cv2.contourArea(c)
                #print(area)
                areas.append(area)
                estimated_weight= 0.012*area+2.321
                # 0.0001から0.001の範囲でランダムな値を加える
                estimated_weights.append(estimated_weight)
                #random_value = random.uniform(0.00001, 0.0001)
                #result = estimated_weight not in estimated_weights
                #if (result):
                    #estimated_weights.append(estimated_weight)
                #else:
                   # estimated_weights.append(estimated_weight+random_value)
                #print(f"{instance_number}, area{instance_number}: {area}")
                #print(f"{instance_number}, {estimated_weight}")
                # csvファイルに保存
                '''if instance_number == 1:
                    my_columns_list = ['instance_number', 'area','estimated_weight','mask']
                    my_columns_str = ','.join(my_columns_list)
                    f.write(my_columns_str + '\n')
                    my_data_list = [str(instance_number), str(area),str(estimated_weight),f'mask_{mask_number}.png']
        ...
        np.savetxt('estimated_weights.txt', estimated_weights)
        ...
        #for mask in masks_color:
            #mask_np_1=mask.to('cpu').detach().numpy().copy()
            #print(mask_np.dtype)
            #mask_np_1=mask_np_1.astype(np.uint8)
        #cv2.imshow("mask",mask_np_1)
        #cv2.waitKey(0)
        #print(masks_color.shape)
        #print('maskshape', (masks.shape))
    ...
    if args.display_text or args.display_bboxes:
        estimated_weights_position=[]
        l=[]
        E_C=[]
        np.savetxt('num_dets_to_consider.txt', np.array([num_dets_to_consider]))
        np.savetxt('instance_number.txt', np.array([instance_numbers]))
        for j in reversed(range(num_dets_to_consider)):
            x1, y1, x2, y2 = boxes[j, :]
            color = get_color(j)
            score = scores[j]
            area=areas[j]
            estimated_weight=estimated_weights[j]
            mask_np = mask_nps[j]
            mask_np=mask_np.astype(np.uint8)
            #cv2.imshow("mask_np",mask_np)
            #cv2.waitKey(0)                     
            instance_number = instance_numbers[j]  
            mask_number = mask_numbers[j]
            k="g"

            #filename = "../yumenoka/result24/data_strawberry1.png"
            #image = cv2.imread(filename, -1)

            #sys.exit()
            #binary=cv2.imread(f"../yumenoka/result26/mask_{mask_number}.png",-1)
            #mask_np=mask_np.reshape((1080,1920))
            mask_np=mask_np.reshape((480,640))
            binary=(mask_np)
            print(type(binary))
            #cv2.imshow("binary",binary)
            #cv2.waitkey(0)
            #white_area=cv2.countNonZero(binary)
            #ret, binary = cv2.threshold(mask_np, 240, 245, cv2.THRESH_BINARY_INV)
            #masked_image = cv2.bitwise_and(img_numpy, img_numpy, mask=mask_np) # 元画像 マスク画像
            
            masked_image = cv2.bitwise_and(img_numpy, img_numpy, mask=binary) # 元画像 マスク画像
            #masked_image = masked_image.astype(np.uint8)

            #masked_image: イチゴが元画像でマスク（くりぬかれた画像）
            #HSV空間に変換 赤色を出す
            img_hsv_1 = cv2.cvtColor(masked_image, cv2.COLOR_BGR2HSV)
            img_hsv_2 = copy.deepcopy(img_hsv_1)
            # w * h * 3
            #赤を抽出
            low_hsv_min = np.array([0, 64,0])
            low_hsv_max = np.array([14, 255, 255])

            #画像の2値化（Hueが0近辺）
            maskHSV_low = cv2.inRange(img_hsv_1,low_hsv_min,low_hsv_max)

            high_hsv_min = np.array([167, 64,0])
            high_hsv_max = np.array([179, 255, 255])

            #画像の2値化（Hueが179近辺）
            maskHSV_high = cv2.inRange(img_hsv_1,high_hsv_min,high_hsv_max)

            #２つの領域を統合
            hsv_mask1 = maskHSV_low | maskHSV_high
            #cv2.imshow("hsv_mask1",hsv_mask1)
            #cv2.waitKey(0)
            #オレンジを抽出
            low_hsv_min = np.array([11, 64,0])
            low_hsv_max = np.array([20, 255, 255])

            #画像の2値化（Hueが0近辺）
            hsv_mask3 = cv2.inRange(img_hsv_1,low_hsv_min,low_hsv_max)

            #high_hsv_min = np.array([169, 64,0])
            #high_hsv_max = np.array([179, 255, 255])
            ...
            #濃い赤を抽出
            low_hsv_min = np.array([0, 64,0])
            low_hsv_max = np.array([10, 255, 90])

            #画像の2値化（Hueが0近辺）
            maskHSV_low = cv2.inRange(img_hsv_1,low_hsv_min,low_hsv_max)

            high_hsv_min = np.array([169, 64,0])
            high_hsv_max = np.array([179, 255, 90])

            #画像の2値化（Hueが179近辺）
            maskHSV_high = cv2.inRange(img_hsv_1,high_hsv_min,high_hsv_max)

            #２つの領域を統合
            hsv_mask4 = maskHSV_low | maskHSV_high

            # 緑の領域を取得してマスク
            hsv_min = np.array([22, 45, 0])
            hsv_max = np.array([130, 255, 255])
            maskHSV = cv2.inRange(img_hsv_2,hsv_min,hsv_max)
            print(type(maskHSV))
            #mask_np=mask_np.reshape((480,640))
            #type(maskHSV)
            #type(binary)
            hsv_mask_2 = maskHSV ^ binary
            white_area=cv2.countNonZero(hsv_mask_2)
            #画像のマスク（合成）
            #resultHSV = cv2.bitwise_and(img_numpy, img_numpy, mask=hsv_mask_2)
            #cv2.imshow("resultHSV",resultHSV)
            #cv2.waitKey(0)
            # resultHSV 0以外で resultsHSV[np.where(resultsHSV > 0)][:,:,0]
            # h = np.mean(resultHSV[:,:,0]) # H 0の方が赤い　180も赤い
            # s = np.mean(resultHSV[:,:,1]) # S
            # v = np.mean(resultHSV[:,:,2]) # V
            #cv2.imshow("hsv_mask1",hsv_mask1)
            #cv2.waitKey(0)
            #cv2.imshow("hsv_mask2",hsv_mask_2)
            #cv2.waitKey(0)
            # 赤い領域/マスクの領域 割合を出す
            # 白いピクセルを数える
            # len()

            if (cv2.countNonZero(hsv_mask3)/white_area)*100>=88 :
                c="Ripe"
                d="オレンジ色,Ripe(完熟)"
                color1=[0,0,255]
                #BGR用
                color1=[255,0,0]
               # print(instance_number,d)
               # print("熟している割合",round((cv2.countNonZero(hsv_mask3)/white_area)*100,1),"%")

            elif (cv2.countNonZero(hsv_mask4)/white_area)*100>=88 :
                c="OverRipe"
                d="濃い赤色,OverRipe(傷みあり)"
                #color1=[60,20,139]
                color1=[0,0,0]
                #print(instance_number,d)
                #print("熟している割合",round((cv2.countNonZero(hsv_mask4)/white_area)*100,1),"%")
            elif (cv2.countNonZero(hsv_mask1)/white_area)*100>=88 :
                c="Ripe"
                d="赤色，Ripe(完熟)"
                color1=[0,0,255]
                #BGR用
                color1=[255,0,0]
                #print(instance_number,d)
                #print("熟している割合",round((cv2.countNonZero(hsv_mask1)/white_area)*100,1),"%")
            elif (cv2.countNonZero(hsv_mask1)/white_area)*100>=60 :
                c="HalfRipe"
                d="HalfRipe(半熟)"
                #color1=[255,0,0]
                #BGR用オレンジ
                ...
                #print(instance_number,d)
                #print("熟している割合",round((cv2.countNonZero(hsv_mask1)/white_area)*100,1),"%")
            elif (cv2.countNonZero(hsv_mask3)/white_area)*100>=60 :
                c="HalfRipe"
                d="HalfRipe(半熟)"
                #color1=[255,0,0]
                #BGR用オレンジ
                ...
                #print(instance_number,d)
                #print("熟している割合",round((cv2.countNonZero(hsv_mask3)/white_area)*100,1),"%")
            elif (cv2.countNonZero(hsv_mask4)/white_area)*100>=60 :
                c="HalfRipe"
                d="HalfRipe(半熟)"
                #color1=[255,0,0]
                #BGR用オレンジ
            ...
            #cv2.imshow("mask",mask_np)
            #cv2.waitKey(0)
            #white_area=cv2.countNonZero(mask_np)
            #ret, binary = cv2.threshold(mask_np, 240, 245, cv2.THRESH_BINARY_INV)
            #masked_image = cv2.bitwise_and(img_org, img_org, mask=mask_np) # 元画像 マスク画像            　　
            #masked_image: イチゴが元画像でマスク（くりぬかれた画像）
            #HSV空間に変換 赤色を出す
            #img_hsv = cv2.cvtColor(masked_image, cv2.COLOR_BGR2HSV)
            # w * h * 3
            #赤を抽出
            #low_hsv_min = np.array([0, 64,0])
            #low_hsv_max = np.array([10, 255, 255])
            ...
            #画像の2値化（Hueが179近辺）
            #maskHSV_high = cv2.inRange(img_hsv,high_hsv_min,high_hsv_max)

            #２つの領域を統合
            #hsv_mask = maskHSV_low | maskHSV_high

            #画像のマスク（合成）
            #resultHSV = cv2.bitwise_and(img_hsv, img_hsv, mask=hsv_mask)
            #cv2.imshow("resultHSV",resultHSV)
            #cv2.waitKey(0)
            # resultHSV 0以外で resultsHSV[np.where(resultsHSV > 0)][:,:,0]
            # h = np.mean(resultHSV[:,:,0]) # H 0の方が赤い　180も赤い
            ...
            #print((len(hsv_mask[np.where(hsv_mask > 0)])/white_area)*100,"%")
            #len(binary[np.where(binary > 0)]
            #image = cv2.resize(img_org,(576,384))
            #viz_image = cv2.resize(hsv_mask,(576,384))
            #cv2.imshow("image", image)
            #cv2.imshow("mask", viz_image)
            #cv2.waitKey(0)


            # hに閾値を設けて 0に近いもしくは180に近いほど赤 very_red を表示
            if estimated_weight>=20.0 and estimated_weight<200:
                e="L"
            elif estimated_weight<20.0 and estimated_weight>17.0:
                e="M"
            else:
                e="S"

            E_C.append([e,c,color1])

            if estimated_weight>=20.0 and estimated_weight<200 and c=="Ripe":
                items.append([estimated_weight,3])
            elif estimated_weight<20.0 and estimated_weight>=17.0 and c=="Ripe":
            ...
            if args.display_text:
                ...
                if _class == "R_strawberry":


                    #text_str = '%s:%.2f' % (instance_number,area) if args.display_scores else _class
                    text_str = '%s:%s' % (e,c) if args.display_scores else _class
                    #text_str = '%s' % (c) if args.display_scores else _class
                    text_str1 = '%.1f%s' % (estimated_weight,k) if args.display_scores else _class
                    ...
                    estimated_weights_position.append([estimated_weight,x1,y1,x2,y2])                  
                    ...
                    #results = hands.process(img_numpy)
                    '''
                    if results.multi_hand_landmarks:
                        for hand_landmarks in results.multi_hand_landmarks:
                            for i, lm in enumerate(hand_landmarks.landmark):
                                ...
                                cv2.circle(img_numpy, (cx, cy), 10, (255, 0, 255), cv2.FILLED)
                            mp_draw.draw_landmarks(img_numpy, hand_landmarks, mp_hands.HAND_CONNECTIONS)

                            # バウンディングボックスの中心座標を計算
                            center_x = (x1 + x2) // 2
                            center_y = (y1 + y2) // 2

                            # 三平方の定理より，中指の第三関節との直線距離を計算
                            distance = np.sqrt((center_x - int(hand_landmarks.landmark[9].x ))**2 + (
                                center_y - int(hand_landmarks.landmark[9].y ))**2)

                            print(distance)

                   '''
        #np.savetxt('items.txt', np.array([items]))
        np.save('items.npy', items)
        #E_C.append([e,c])
        E_C = [(e, c, color1) for e, c, color1 in reversed(E_C)]
        #E_C = [[e, c, color1] for e, c, color1 in reversed(list(zip(E_C)))]
        E_C = np.array(E_C)
        with open('E_C.txt', 'w') as file:

            for item in E_C:
                file.write(f"{item[0]}, {item[1]}, {item[2][0]}, {item[2][1]}, {item[2][2]}\n")
        #np.savetxt('E_C.txt', E_C)                            
        ...
        np.savetxt('lst.txt', np.array([lst]))
        
        #if input("Press Enter to start subprocess: ") == "":
...
def prep_metrics(ap_data, dets, img, gt, gt_masks, h, w, num_crowd, image_id, detections:Detections=None):
    """ Returns a list of APs for this image, with--image=input_image.png:output_image.png each element being for a class  """
        ...
        try:
            ...
            while running:
                ...
                if not frame_buffer.empty():
                    ...
                    if out_path is None:
                        # 現在のフレームを取得
                        frame_current = frame_buffer.get()
                        # 画像の縦横比を取得
                        h, w = frame_current.shape[:2]
                        # 変更する縦の値を計算
                        h_trans = round(h * (w_trans/w))

                        dst = cv2.resize(frame_buffer.get(), dsize=(w_trans,h_trans))
